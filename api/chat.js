...
module.exports = function handler(req, res) {
  ...
  req.on('end', function() {
    var body;
    try { body = JSON.parse(bodyStr); } catch(e) { res.status(400).json({ error: 'Invalid JSON' }); return; }
    ...
    var apiReq = https.request(options, function(apiRes) {
      ...
      apiRes.on('end', function() {
        ...
        catch(e) { res.status(500).json({ error: 'Invalid response', raw: data.slice(0, 200) }); }
      });
    });
